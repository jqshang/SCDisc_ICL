...
RF = '/home/rfaulk/projects/aip-rgrosse/rfaulk/SCDisc_ICL/data/semeval_en'
...
def main():
    ...
    if not os.path.exists(f'{RF}/merged/'):
        os.makedirs(f'{RF}/merged/')

    with open(f'{RF}/merged/all.jsonlist', 'w') as f:
    ...
    with open(f'{RF}/merged/all.jsonlist') as f:
    ...
    with open(f'{RF}/merged/pre_lemmatized_all.jsonlist',
              'w') as f:
    ...
    truth_dict = {}
    with open(f'{RF}/raw/truth/graded.txt', 'r') as f:
        lines = f.read().split('\n')
        for line in lines:
            if len(line) > 0:
    ...
    target_lemma_to_token = {
        'attack_nn': ['attack', 'attacks'],
        'bag_nn': ['bag', 'bags'],
        'ball_nn': ['balls', 'ball'],
        'bit_nn': ['bit', 'bits'],
    ...
    with open(f'{RF}/token_to_lemma.json', 'w') as f:
