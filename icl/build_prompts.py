...
def build_prompt(
  ...
  if icl_examples:
    ...
    for ex in icl_examples:
      ...
      block = format_word_block(
          ex["word"], p1_sents, p2_sents, cfg, label=ex["label"]
      )
      parts.append(block)
      parts.append("")
...
def sample_icl_bucket(
  ...
  changed = [
      w
      for w, s in annotated_words.items()
      if s > threshold and cfg["period_1"] in contexts.get(w.split("_")[0], {})
  ]
  unchanged = [
      w
      for w, s in annotated_words.items()
  ...
  for w in selected_pos:
    clean_w = w.split("_")[0]
    examples.append(
        {"word": clean_w, "contexts": contexts.get(clean_w, {}), "label": "Yes"}
    )
  for w in selected_neg:
    clean_w = w.split("_")[0]
    examples.append(
        {"word": clean_w, "contexts": contexts.get(clean_w, {}), "label": "No"}
    )
...
def main():
  ...
  parser.add_option("--tokenizer-model", type=str, default="bert-base-uncased")
  ...
  print(f"Built {len(prompts)} prompts ({len(icl_examples)} ICL examples each)")
